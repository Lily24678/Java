...
public class HashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
	...
	/**
	 * DEFAULT_INITIAL_CAPACITY 是： 当前数组的初始长度 2^4 = 16 map默认的长度
	 */
	static final int DEFAULT_INITIAL_CAPACITY = 1 << 4; // aka 16

	/**
	 * MAXIMUM_CAPACITY: map的最大允许的长度 2^30
	 */
	 ...
	 * 
	 * 当前默认的长度是 16 什么时候增长？ 16*0.75 = 12 增长原来长度的2的次幂
	 */
	 ...
	 * code of keys to make hash collisions harder to find. If 0 then alternative
	 * hashing is disabled.
	 */
	...
	/**
	 * Initialize the hashing mask value. We defer initialization until we really
	 * need it.
	 */
	 ...
	 * encounter collisions for hashCodes that do not differ in lower bits. Note:
	...
	public V get(Object key) {
		...
		// get 取值： 先将当前的 hash(key) indexFor(hash, table.length) 结合
		// 能够直接定位到当前的key位置（数组的位置）
	...
	public V put(K key, V value) {
		...
		// hash(key) ---> int hash --> indexFor() 5
		int hash = hash(key);
		int i = indexFor(hash, table.length); // 21 & 15 = 5
		// key 得冲突解决
	...
	/**
	 * 将旧的oldTable 重新计算hash的值，在放入新的Entry[] newTable 这就导致的性能相对偏低
	 */
	...
	/**
	 * Special version of remove for EntrySet using {@code Map.Entry.equals()} for
	 * matching.
	 */
